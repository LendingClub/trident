...
import org.lendingclub.trident.envoy.EnvoyRouteDiscoveryDecorator;
...
@Component
public class SwarmRouteDiscoveryDecorator implements EnvoyRouteDiscoveryDecorator {

	Logger logger = LoggerFactory.getLogger(SwarmRouteDiscoveryDecorator.class);
	@Autowired
	NeoRxClient neo4j;
		...
		ctx.newSearch().search().forEach(it ->{
			...
			if (it.getAppId().isPresent()) {
				...
				if (!paths.isEmpty()) {
							...
							String fullyQualifiedCluster = SwarmClusterDiscoveryDecorator.shortenClusterName(String.format("%s--%s--%s--%s", ctx.getServiceZone().orElse("unknown"), ctx.getEnvironment().orElse("unknown"),
