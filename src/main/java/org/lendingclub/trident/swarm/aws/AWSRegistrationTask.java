	...
	class DockerNodeLabeler implements Consumer<JsonNode> {
		...
		public void accept(JsonNode t) {
			...
			try {
				...
				WebTarget wt = getApplicationContext().getBean(SwarmClusterManager.class)
						.getSwarmManagerWebTarget(tridentClusterId);
	...
	class EC2InstanceTagger implements Consumer<JsonNode> {
		...
		public void accept(JsonNode t) {
			try {
				...
				AmazonEC2 ec2 = getApplicationContext().getBean(AWSClusterManager.class).getAccountManager().newClientBuilder(account, AmazonEC2ClientBuilder.class).withRegion(region).build();
