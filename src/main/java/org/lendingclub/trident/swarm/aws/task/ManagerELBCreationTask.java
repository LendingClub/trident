...
public class ManagerELBCreationTask extends DistributedTask {
	...
	protected void maybeCreateElasticLoadBalancer(ObjectNode n) {
		...
		AmazonAutoScaling asgClient = getApplicationContext().getBean(AWSAccountManager.class)
				.getClient(n.get("aws_account").asText(), AmazonAutoScalingClientBuilder.class,n.get("aws_region").asText());
		...
		AmazonElasticLoadBalancing elbClient = awsClusterManager.getAccountManager()
				.getClient(awsAccount, AmazonElasticLoadBalancingClientBuilder.class,awsRegion);

		CreateLoadBalancerRequest createLoadBalancerRequest = createLoadBalancerRequest(tridentClusterId, subnets,
				securityGroups);

		awsClusterManager.getManagerLoadBalancerInterceptors().getInterceptors().forEach(it -> {
		...
		new LoadBalancerCreatedEvent()
				...
				.withMessage(String.format("ELB %s created in account %s", createLoadBalancerRequest.getLoadBalancerName(), awsAccount))
				.withTridentClusterId(tridentClusterId)
				.send();
		...
		try {
			...
			new LoadBalancerAttachedEvent()
				...
				.withMessage(String.format("ELB %s attached to ASG %s in account %s", 
