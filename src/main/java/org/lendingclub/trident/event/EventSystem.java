...
public class EventSystem {
	...
	@Value("${TRIDENT_EVENT_SYSTEM_THREAD_COUNT:50}")
	int threadCount = 50;

	@Value("${TRIDENT_EVENT_SYSTEM_BACKLOG:2048}")
	int backlog = 2048;
	...
	public void post(Object event) {
		...
		if (event instanceof TridentEvent) {
			if (TridentEvent.class.cast(event).sent) {
				throw new IllegalStateException("message already sent: "+event.toString());
			}
			TridentEvent.class.cast(event).sent=true;
		}
	...
	public synchronized void init() {
		...
		reflexBus = ReflexBus.newBuilder().withThreadPoolExecutorConfig(cfg -> {
			cfg.withThreadNameFormat("EventSystem-%d").withThreadPoolSize(threadCount).withThreadTimeout(true)
